...
public class ZoneCommand extends AbstractCommand
{
    ...
    public boolean handlePlayer(TregminePlayer player, String[] args)
    {
        ...
        else if ("setmainowner".equals(args[0])) {
            setMainOwner(player, args);
            return true;
        }
        return false;
    }
    ...
    private void setMainOwner(TregminePlayer player, String[] args)
    {
        ...
        if (args.length < 3) {
            player.sendMessage(RED
                    + "syntax: /zone setmainowner [zone] [player]");
            return;
        }

        String zoneName = args[1];
        String setName = args[2];
        Player userName = Bukkit.getPlayer(args[2]);
        ...
        if (zone.getMainOwner().equalsIgnoreCase(player.getName())){
            zone.delMainOwner();
            zone.setMainOwner(setName);
            userName.sendMessage("You are now the Main Owner in " + zone.getName());
            player.sendMessage(userName.getName() + " is now the Main Owner in " + zone.getName());
        }else{
            player.sendMessage(RED + "You must be an Admin or the Main Owner to execute this command!");
        }
    }
